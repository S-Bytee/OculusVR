...
public class CardPopup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (fallToZero)
        {
            transform.position = Vector3.Lerp(transform.position, new Vector3(-24, 0, startZPos), Time.deltaTime * centeringSpeed);
            transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(Vector3.zero), Time.deltaTime * centeringSpeed);
            if (Vector3.Distance(transform.position, new Vector3(-24, 0, startZPos)) < 0.0025f)
            {
                transform.position = new Vector3(-24, 0, startZPos);
                fallToZero = false;
            }
        }
