...
public class TexturePainter : MonoBehaviour {
	...
	GameObject brushCursor,brushContainer; //The cursor that overlaps the model and our container for the brushes painted
	Camera canvasCam;  //The camera that looks at the model, and the camera that looks at the canvas.
	...
	{ 
		...
		brushCursor = transform.GetChild(1).gameObject;
		brushContainer = GameObject.Find("BrushContainer");
		canvasCam = transform.GetChild(3).gameObject.GetComponent<Camera>();
		
	}

	void Update () {

		brushColor = ColorIndicator.Instance.color.ToColor();   //Updates our painted color with the selected color
	...
	void UpdateBrushCursor(){
		...
		if (HitTestUVPosition (ref uvWorldPosition) && !saving) {
			...
			brushCursor.transform.position =PhysicsPointer.Instance.CalculateEnd();									
		} else {
			brushCursor.SetActive(false);
		}		
	}
	...
	bool HitTestUVPosition(ref Vector3 uvWorldPosition){
		RaycastHit hit;
		hit = PhysicsPointer.Instance.hit;
		Vector3 cursorPos = PhysicsPointer.Instance.CalculateEnd();
		Ray cursorRay=PhysicsPointer.Instance._ray;
